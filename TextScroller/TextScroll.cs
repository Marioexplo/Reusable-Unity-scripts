    ...
    public class TextScroll : MonoBehaviour
    {
        ...
        public bool HasFinished
        {
            get
            {
                return type switch
                {
                    Option.simple => count >= text.Length,
                    Option.array => count >= array[arrayNumber].Length,
                    Option.branches => count >= FindBranch.text.Length,
                    _ => throw new System.Exception("error: type not set"),
                };
            }
        }
        private Branch FindBranch
        {
            get
            {
                foreach (Branch txt in structure) if (txt.name == branch) return txt;
                ...
                if (timer > time)
                {
                    ...
                    switch (type)
                    {
                        ...
                        case Option.branches:
                            if (count < FindBranch.text.Length)
                            {
                                Add();
                                break;
                            }
        ...
        public string GetScrolled()
        {
            return type switch
            {
                Option.simple => text[..count],
                Option.array => array[arrayNumber][..count],
                Option.branches => FindBranch.text,
                _ => throw new System.Exception("error: type not set"),
            };
        }
